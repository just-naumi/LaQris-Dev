...
def normalisasi_teks(teks_mentah):
    if not teks_mentah or teks_mentah == "Tidak terbaca":
        return ""
...
def query_sqlite_reputation(nmid_digital, merchant_name_phys):
    db = SessionLocal()
    try:
...
def process_qris_verification(gambar_input, filename_base="scan"):
    ...
    # 5. Matching & SQLite Reputation
    norm_dig = normalisasi_teks(dig_name)
    norm_phys = normalisasi_teks(phys_name)

    is_match = (dig_name.lower().strip() == phys_name.lower().strip() and dig_name != "") or (norm_dig == norm_phys and norm_dig != "")
    if not is_match and dig_name != "Tidak ditemukan" and phys_name != "Tidak terbaca":
    ...
    is_mismatch = (not is_match) or (dig_nmid != "Tidak ditemukan" and phys_nmid != "Tidak terbaca" and not is_nmid_match)

    reputation = query_sqlite_reputation(dig_nmid, norm_phys)

    risk_level = "HIGH_RISK" if is_mismatch or (reputation and reputation["rating"] <= 3.0) else "SAFE"
